...
namespace ExampleApp
{
    namespace PoiCreation
    {
        namespace PoiRing
        {
            ...
            PoiRingController::PoiRingController(IPoiCreationModel& poiCreationModel,
            ...
            void PoiRingController::Update(float dt, const Eegeo::Camera::RenderCamera& renderCamera, const Eegeo::dv3& cameraEcefInterestPoint)
            {
                ...
                if (m_poiCreationModel.GetCreationStage() == Inactive && m_scaleInterpolationParam < 0.01f)
                {
                    m_poiCreationModel.SetPosition(cameraEcefInterestPoint);
                }
                ...
                Eegeo::dv3 cameraRelativePosition = m_poiCreationModel.GetPosition() - renderCamera.GetEcefLocation();
                ...
                Eegeo::v3 up = m_poiCreationModel.GetPosition().Norm().ToSingle();
                const float iconHeightInMeters = 150.f * transitionScale;
                Eegeo::dv3 iconPosition = m_poiCreationModel.GetPosition() + (up * iconHeightInMeters);
            ...
            {
                float delta = m_poiCreationModel.GetCreationStage() == Ring ? dt : -dt;
