    ...
    namespace OfflineRouting
    {
        namespace SdkModel
        {
                                                     ...
                                                     std::vector<RoutingEngine::OfflineRoutingFindPathResult>& pathResults)
            {
                ...
                {
                    const RoutingEngine::OfflineRoutingFindPathResult& result = m_offlineRoutingPathFinder.FindPath(queryPoints.at(i), queryPoints.at(i+1), transportationMode);
            ...
            void OfflineRoutingService::ProcessQueries()
            {
                for (const auto& query : m_queriesToProcess)
                {
                    ...
                    if (!TryFindPaths(queryPoints, options.transportationMode, pathResults))
                    {
                        NotifyQueryFailed(routingQueryId);
                        continue;
                    }
