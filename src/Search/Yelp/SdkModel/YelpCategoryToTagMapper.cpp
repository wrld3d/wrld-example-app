    ...
    namespace Search
    {
        namespace Yelp
        {
            namespace SdkModel
            {
                ...
                YelpCategoryToTagMapper::YelpCategoryToTagMapper(const std::shared_ptr<Eegeo::Web::IWebLoadRequestFactory>& webRequestFactory,
                ...
                {
                    Eegeo_ASSERT(m_yelpMappingData != nullptr);
                    Eegeo_ASSERT(m_webRequestFactory != nullptr);
                        ...
                        std::string& out_bestMatchedAppTag)
                {
                    ...
                    else
                    {
                        ...
                        for(std::vector<std::string>::const_iterator it = yelpCategories.begin(); it != yelpCategories.end(); ++ it)
                        {
                            ...
                            if(yelpLeafToFoundationIt != m_yelpLeafCategoryToYelpFoundationCategoryMap.end())
                            {
                                ...
                                // TODO: this needs repositorying up...
                                const mapIt yelpFoundationategoryToAppCategoryIt(m_yelpMappingData->yelpFoundationCategoryToAppTag.find(yelpFoundationCategory));
                                
                                if(yelpFoundationategoryToAppCategoryIt != m_yelpMappingData->yelpFoundationCategoryToAppTag.end())
                                {
                    ...
                    out_bestMatchedAppTag = m_yelpMappingData->defaultAppTag;
                    return false;
                }
                ...
                void YelpCategoryToTagMapper::ParseYelpCategoryTaxonomy(const std::string& serializedTaxonomyJson)
                {
                    ...
                    if (!document.Parse<0>(serializedTaxonomyJson.c_str()).HasParseError())
                    {
                        ...
                        for(size_t i = 0; i < numFoundationCategories; ++i)
                        {
                            ...
                            RecursivelyMapYelpCategoryTaxonomySubHierarchyToFoundationCategory(subTree,
                                                                                               foundationCategory,
                                                                                               m_yelpMappingData->yelpFoundationCategoryToAppTag,
