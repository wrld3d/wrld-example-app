...
namespace ExampleApp
{
    namespace Search
    {
        namespace Combined
        {
            CombinedSearchServiceModule::CombinedSearchServiceModule(std::map<std::string, SdkModel::ISearchServiceModule*> searchServiceModules)
            : m_pSearchService(NULL)
            {
                std::map<std::string,SdkModel::ISearchService*> searchServices;
                
                for(std::map<std::string, SdkModel::ISearchServiceModule*>::const_iterator it = searchServiceModules.begin(); it != searchServiceModules.end(); ++it)
                {
            ...
            SdkModel::ISearchService& CombinedSearchServiceModule::GetSearchService() const
            {
