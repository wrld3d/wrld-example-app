		...
		namespace Decarta
		{
			...
			void DecartaSearchService::CancelInFlightQueries()
			{
				if(m_pCurrentRequest != NULL)
				{
					...
					SearchQuery* pRequestBoundQuery = static_cast<SearchQuery*>(m_pCurrentRequest->GetUserData());
			...
			void DecartaSearchService::PerformLocationQuerySearch(const SearchQuery& searchQuery)
			{
				CancelInFlightQueries();
			...
			void DecartaSearchService::IssueRequest(const std::string queryUrl, const SearchQuery& query)
			{
				...
				SearchQuery* pRequestBoundQuery = Eegeo_NEW(SearchQuery)(query.Query(),
				                                  query.IsCategory(),
				                                  query.Location(),
				                                  query.Radius());
			...
			void DecartaSearchService::HandleSearchResponse(Eegeo::Web::IWebLoadRequest& webLoadRequest)
			{
				...
				{
					std::vector<SearchResultModel> results;
					SearchQuery* pRequestBoundQuery = static_cast<SearchQuery*>(webLoadRequest.GetUserData());
