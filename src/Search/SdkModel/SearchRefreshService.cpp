            ...
            void SearchRefreshService::TryRefreshSearch(float deltaSeconds, const Eegeo::dv3& interestPointEcef, const Eegeo::dv3& viewpointEcef)
            {
                ...
                if (shouldRefresh)
                {
                    ...
                    if (previousQuery.IsTag() && previousQuery.Query() == Search::Swallow::SearchConstants::OFFICE_CATEGORY_NAME)
                    {
                        ...
                        m_searchQueryPerformer.AskForQueryRefresh(previousQuery, false, currentLocation, previousQuery.Radius());
                    }
                    else
                    {
                        m_searchQueryPerformer.AskForQueryRefresh(previousQuery, false, currentLocation);
                    }
