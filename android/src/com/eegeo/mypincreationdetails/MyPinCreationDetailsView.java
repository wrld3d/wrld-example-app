...
public class MyPinCreationDetailsView implements View.OnClickListener, IActivityIntentResultHandler {
	...
	private void createView() {
		final RelativeLayout uiRoot = (RelativeLayout) m_activity.findViewById(R.id.ui_container);
		m_view = (ScrollView) m_activity.getLayoutInflater().inflate(R.layout.poi_creation_details_layout, uiRoot,
				false);
		...
		// m_scrollSection =
		// (ScrollView)m_view.findViewById(R.id.poi_creation_details_scroll_section);
	...
	public void dismiss() {
		m_view.setVisibility(View.GONE);
		// Hiding the keyboard if open
		if (m_activity.getCurrentFocus().getWindowToken() != null) {
			InputMethodManager inputManager = (InputMethodManager) m_activity
					.getSystemService(Context.INPUT_METHOD_SERVICE);
			inputManager.hideSoftInputFromWindow(m_activity.getCurrentFocus().getWindowToken(), 0);
		}
	}
	...
	public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
		...
		} else if (requestCode == PhotoIntentDispatcher.SELECT_PHOTO_FROM_GALLERY
		...
		if (requestCode == PhotoIntentDispatcher.SELECT_PHOTO_FROM_GALLERY
				|| requestCode == PhotoIntentDispatcher.REQUEST_IMAGE_CAPTURE) {
			m_awaitingIntentResponse = false;
		}
	}
		...
		if (m_shouldShareButton.isChecked() && !m_hasNetworkConnectivity) {
			if (!m_showingNoNetworkDialog) {
				...
				builder.setTitle("No network connection")
						...
						.setCancelable(false).setPositiveButton("Dismiss", new DialogInterface.OnClickListener() {
